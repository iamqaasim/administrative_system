...
def employee_edit():
  ...
  # options data
  all_departments = list(collection_2.find())
  ...
  return render_template("details.html", operation = "Edit", section= "Employee", user=user, all_departments=all_departments, all_managers=all_managers)
...
def employee_list():
  ...
  if "user" in session:
    ...
    manager_session = session['user']
    manager_data = collection_1.find_one({"employee_login.username": manager_session})
    manager_full_name = f"{manager_data['employee_details']['first_name']} {manager_data['employee_details']['last_name']}" 
    
    all_departments = list(collection_2.find({'manager': manager_full_name}))
    # check manager or employee
    employee_data = collection_1.find_one({'employee_login.username': session["user"] })
  ...
  if request.method == 'POST':
    if user == "user":
      ...
      data_list = employee_list_data(request_department, request_status, manager_full_name)
      return render_template("list.html", section= "Employee", user=user, data_list=data_list, list_length=len(data_list), all_departments=all_departments, all_managers=all_managers)
...
def department_edit():
  ...
  return render_template("details.html", operation = "Edit", section= "Department", user=user, all_departments=all_departments, all_managers=all_managers)
...
def department_list():
  ...
  return render_template("list.html", section= "Department", user="super_user", data_list=data_list, list_length=len(data_list), all_managers=all_managers)
